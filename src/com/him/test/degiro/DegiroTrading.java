...
import com.him.test.degiro.InputFormat.PileOfFlut;
...
public class DegiroTrading {
	...
	private static void processEachTestCase(InputFormat input) {
		for (PileOfFlut pileOfFluts : input.getPileOfFluts()) {
			processEachLine(pileOfFluts);
		}

	}

	private static void processEachLine(PileOfFlut pileOfFluts) {
	...
	private static List<InputFormat> readInput() {
		...
		while (!line.startsWith("0")) {
			...
			List<PileOfFlut> plieOfFlutsList = new ArrayList<>();
			for (int i = 0; i < noOfSchurrs; i++) {
				PileOfFlut pileOfFLut = input.new PileOfFlut();
